...
LB5<-KD %>% group_by(Year_2, Region) %>% summarize(Waste_3=sum(WasteGen))

LB6<-LB5 %>% inner_join(LA5, 
        by=c('Year_2'="Year_2","Region"="Region")) 

LB6<-LB6%>% 
  mutate(EOL_2=Waste_3*(Percentage/100))
...
LA5$Region<-as.factor(LA5$Region)
LA5$EOL<-as.factor(LA5$EOL)
LA6$Region<-as.factor(LA6$Region)
LA6$EOL<-as.factor(LA6$EOL)
levels(LA5$Region)
str(LA5)

# Predicting future EOL percentages by first calculating mass in Metric tons. 
# still does not work 

LB7<-LB6 %>%
  group_by(Region, EOL) %>%
  nest() %>%
  inner_join(LA6 %>% select(-EOL_2) %>% 
               group_by(Region, EOL) %>% nest(),
             by = c("Region", "EOL")) %>%
  mutate(model = data.x %>% map(~lm(EOL_2 ~ Year_2, data=.)),
         EOL_2 = map2(model, data.y, predict)) %>%
  select(-data.x, -model) %>%
  unnest() %>%
  bind_rows(LA6, .)
...
MA5<-MA5 %>% arrange(desc(Year_2)) %>%
  group_by(Region, EOL) %>% 
  fill(Percentage, .direction = 'down')


MA6<-LB5 %>% inner_join(MA5, 
        by=c('Year_2'="Year_2","Region"="Region")) %>% 
  mutate(EOL_Waste_5=Waste_3*(Percentage/100))

# Sending Recycling Forward 
MA6.1<-MA6 %>% filter(EOL=="Recycling")%>% 
  select(Region,EOL_Waste_5) %>% group_by(Region) %>% 
  mutate(Year_3=c(1974:2053))

MA6.2<-MA6.1 %>% arrange(desc(Year_3)) %>% 
  dplyr::slice(-1:-3) %>% 
  select(-Year_2)%>% 
  mutate_at(3, round, 2)

colnames(MA6.2)[colnames(MA6.2) == 'EOL_Waste_5'] <- 'Secondary_Waste'

MA7<-MA6 %>% inner_join(MA6.2,
                   by=c("Region"="Region","Year_2"="Year_3")) %>% 
  filter(EOL=="Recycling") %>% 
  mutate(EOL_Waste_5=EOL_Waste_5+Secondary_Waste) %>% 
  select(Year_2, Region, EOL, EOL_Waste_5)

MA8<-MA6 %>% filter(EOL!="Recycling") %>%
  select(Year_2,Region,EOL,EOL_Waste_5)
  
# Final Total EOL including Recycling carried forward! 
MA9<-rbind(MA8,MA7)
```
...
#MA8<-KD %>% arrange(desc(Year_2)) %>% 
  filter(Major_Market_Sector=="Electrical_Electronic")

#MA9<-MA8[c(109:204),]

#write.csv(MA9, "/Users/dr.nivedita/Library/CloudStorage/OneDrive-Personal/04_Post-Doc/01.R_Code/06_Output\\MA9.csv")
