...
AC_01_China<-read.csv(glue("{one_drive_path}/02_Data_Raw/08.1_China_End_Use_and_Type.csv"))
AC_02_NAFTA<-read.csv(glue("{one_drive_path}/02_Data_Raw/09.1_NAFTA_End_Use_and_Type.csv"))
AC_03_Europe<-read.csv(glue("{one_drive_path}/02_Data_Raw/10.1_Europe_End_Use_and_Type.csv"))
AC_04_Row<-read.csv(glue("{one_drive_path}/02_Data_Raw/11.1_RoW_End_Use_and_Type.csv"))
...
KD<-rbind(KC_1Transport,KC_2Packaging,KC_3Building,
          KC_4Electric,KC_5House,KC_6Agri,KC_7Other)
...
# Waste_Gen_2 is waste for each polymer type 
KD2 
 
# Sam's ask 2 = Consumption - year, region, market_sector
# PC_sector is consumption 
KA3
 
# Sam's ask 3 = Waste Gen - year, region, EOL type
# EOL_Waste_5 is waste for each EOL fate 
MA6 
 
#Sam's ask 4 = Consumption - year, region, market_sector, polymer_type
# PC_type is plastic consumption by type of polymer 
MA7<-KA3 %>% inner_join(D1, 
        by=c('Major_Market_Sector'="Major_Market_Sector","Region"="Region")) %>% 
  filter(Type!="Sum") %>% 
  mutate(PC_Type=PC_Sector*Percentage)
