...
date: "2023-08-7th" August 7th 
output: html_document

Final with textiles - as in polymer Data updated 
---
...
# Projecting Trade forward 

```{r}
G8.1<-G8 %>% select(-Converter_Consump_2)
...
# If you want to forward project Trade 
...
#write.csv(G8.1,"/Users/dr.nivedita/Library/CloudStorage/OneDrive-Personal/04_Post-Doc/01.R_Code/06_Output\\G8.1.csv")
...
#sum(KD4$Prim_Waste) = #25275

#KD4 %>% group_by(Region) %>% 
  #summarise(sum(Prim_Waste))

#ggplot(data = KD4, aes(x = Year_2, y = Prim_Waste)) +
  #geom_point()
...
This needs to be changed in the future for the publication 
...
#Prim_W_EOL= Sec_Waste
MA5.1<- MA5.1 %>% mutate(Prim_W_EOL=Prim_Waste*(Percentage/100))
...
# Setup calculations
get_t_waste <- function (target) {target$Prim_W_EOL + target$Sec_Waste }
...
#write.csv(Target_MB_2, "/Users/dr.nivedita/Library/
#CloudStorage/OneDrive-Personal/04_Post-Doc/01.R_Code/06_Output\\Target_MB_2.csv")

```

## Recaculating Total Waste and Actial EOL 

```{r}

TBA1<-Target_MB_2 %>% 
  select(Year_2, Region, T_Waste)

TBA2<-KD4 %>% inner_join(TBA1,
                   by=c("Region"="Region","Year_2"="Year_2")) %>% 
  select(-EOL) %>%
  arrange(desc(Year_2)) %>% 
  group_by(Region) %>% 
  mutate(Total_Waste=Prim_Waste+T_Waste)


TBA3<-TBA2 %>% select(Year_2, Region, Total_Waste)

TBA4<-TBA3 %>% inner_join(MA5.1,
                   by=c("Region"="Region","Year_2"="Year_2")) %>% 
  select(-Prim_Waste, -Prim_W_EOL)

TBA5<- TBA4 %>% mutate(Real_EOL=Total_Waste*(Percentage/100))

TBA6<-TBA5 %>%  select(-Total_Waste, -Percentage)
  

```


# WASTE BY SECTOR AND POLYMER TYPE 
...
KD2.1<-KD2 %>% select(-WasteGen, -Percentage)
...
levels(TB3$Major_Market_Sector)
TB3$Major_Market_Sector<-as.factor(TB3$Major_Market_Sector)
...
# PC_sector is consumption in Million Metric Tons #J_02 is the precursor to this. 
# PC Polymer is Consumption by Polymer 
TB4

# END OF LIFE 1 - Here we don't know how much is recycled or incinerated etc. 
# Sam's ask 1 = Waste Gen - year, region, market_sector, polymer type
# Waste_Gen_2 is waste for each polymer type in Million Metric Tons # KD2.1
KD3 

# END OF LIFE 2 - Here we do not know how much is from what sector - OR Polymer, but we know how much is recycled etc. 
# Sam's ask 3 = Waste Gen - year, region, EOL type
# Real_EOL_ is waste for each EOL fate in Million Metric Tons
TBA6
